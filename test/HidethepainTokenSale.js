...
contract("HidethepainTokenSale", async (accounts) => {
	let token, tokenSale;
	const tokenPrice = 225000000000000; //in wei
	const admin = accounts[0];
	const buyer = accounts[1];
	const tokensAvailable = 500000;
	var numberOfTokens = 10;

	before(async () => {
		token = await HidethepainToken.new(1000000);
		tokenSale = await HidethepainTokenSale.new(token.address, tokenPrice);
